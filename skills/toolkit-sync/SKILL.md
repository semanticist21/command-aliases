...
Public source is `semanticist21/agent-toolkit`. Canonical managed skills live in `skills/`; Codex, Claude, and OpenCode copies and version markers must match it. Run `scripts/toolkit-sync check` before writes and `scripts/toolkit-sync sync` after landing. After a live sync, continue with `$environment-sync apply user`; the toolkit workflow is incomplete until every private-baseline critical SSH target passes its registered root-capability probe. The file-copy script remains environment-agnostic and transactional; the invoking agent owns this second phase and reports partial completion precisely.
...
direct root SSH. During this second phase it must not invoke toolkit
