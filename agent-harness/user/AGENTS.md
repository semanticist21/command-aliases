...
- For Kobbokkom operations, use `gh` for GitHub, `wrangler` for Cloudflare, and `gcloud` for Google Cloud. `synology-kkomjang` is a restricted, non-interactive alias for `$secrets-sync` transfers only (`scp -O`); use a separate NAS administrator connection only when the relevant project documentation authorizes it. Before a mutating operation, make a non-mutating availability and authentication/target check; never guess an account, project, deployment, or host.
