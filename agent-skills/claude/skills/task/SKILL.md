...
```bash
node ~/.claude/skills/task/scripts/task-worktree-plan-cleanup.mjs --repo "<caller-root>" --worktree "<task-worktree>" --branch <task-branch> --head <creation-commit>
```
...
```bash
node ~/.claude/skills/task/scripts/task-worktree-create.mjs <slug> --id <unique-id> --repo <repo-root> --summary "<task summary>"
```
...
working tree as read-only task context unless the user explicitly asks to apply changes there.
...
acceptance criteria. Use `/goal` before planning to create or select the task goal; if an unrelated goal is
active, report the conflict before changing it. Report progress in normal task updates, not goal status
commands, unless the platform explicitly supports non-terminal progress states.

Mark the goal complete only after QA is clean or valid/actionable findings are zero **and** commit,
merge-back/cleanup, and owned queue drain are all done. Never mark blocked work, budget exhaustion, or
partial progress complete. Mark it blocked only when the same blocker has repeated across the required goal
turns and no meaningful progress is possible without user input or external state change — a dirty caller
tree is not such a blocker.
...
3. UI changes require live browser verification, not code inspection alone. Load the Chrome control skill,
   open or reload the changed route, and inspect the actual rendered screen with DOM or screenshot evidence.
   If that path is unavailable after its documented retry/recovery steps, do not silently substitute another
