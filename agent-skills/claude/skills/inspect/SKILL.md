...
tool / subagents for fan-out; in Codex use its subagent/agent mechanism. Where
neither is available, run FIND→VERIFY→GATE sequentially in the main thread with the
ledger as external memory. The gate and the read-only boundary hold on every runtime.
