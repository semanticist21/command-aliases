...
If production data exists, the platform has its own migration runner, or SQL is not the primary data model, settle that lifecycle with `$grill-me` before `$harness` documents or structures it.
