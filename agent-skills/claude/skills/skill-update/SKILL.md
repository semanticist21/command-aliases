...
1. **Resolve the target.** Take the skill name from the args; if absent, infer it
   from what the user described and confirm. Glob all four roots above for
...
2. **Read every copy.** They should be identical except for tool-name wording
   (e.g. Claude's `Explore`/`WebSearch` vs a generic phrasing for Codex). Note any
   pre-existing drift so you do not silently overwrite an intentional difference.
