...
- **Codebase**: relevant files, prior art, existing decisions in `AGENTS.md` /
  `docs/`. Use code-search subagents for local truth.
- **Web**: web search + fetch for current docs, releases, benchmarks,
...
Each angle is blind to the others by design. For heavy topics, run a fan-out
(multi-modal sweep → adversarial verify → synthesize); this skill is the lighter
inline path.
