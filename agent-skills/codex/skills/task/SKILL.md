...
Carry a development task from request to verified result. Default to **subagents** so
...
- **If requirements are genuinely unclear or contradictory** — not just "could use a
  default" — use a grill-me / adversarial-interrogation skill to pressure-test the
  request before building. Skip grilling for clear or low-stakes tasks; pick sensible
  defaults and note them.
- **If the task needs external or unfamiliar knowledge** (a library's real API, a
  spec, a design decision to vet, a claim to verify) — use a research skill and fold
  its conclusion into the plan. Skip when the codebase already answers it.

### 1. Plan (SKIPPABLE)

- Skip for small, obvious, bounded edits — go straight to Implement.
- Otherwise spawn a planning agent to read the relevant code and return a concrete
  step-by-step plan: files to touch, order, edge cases, risks.
